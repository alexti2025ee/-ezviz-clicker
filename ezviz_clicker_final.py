...
# -------------------------------
# Вывод окна EZVIZ на передний план перед кликом
# -------------------------------
...
def click_points(points, offset_x, offset_y):
    for x, y in points:
        ...
        # Второй клик (подстраховка, если первый не был обработан приложением)
        pyautogui.mouseDown()
        time.sleep(0.15)
        pyautogui.mouseUp()
...
def run_check(template):
    try:
        ...
        img, left, top, method = capture_window(hwnd)
        ...
        if points:
            logging.info(f"Найдено кнопок: {len(points)} (метод: {method})")
            notify_text = f"{NOTIFY_TEXT} ({method})"
